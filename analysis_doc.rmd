...
The working directory is set followed by downloading the dataset from its url and therefore unzipping the file to "step_data.csv".
The data comes directly from Roger Pengs [github account](https://github.com/rdpeng/RepData_PeerAssessment1).
...
The variable names and the structure of the file are given by as follows:
```{r activity details}
names(activity)
str(activity)
...
At the point the format of the file is ready for analysis. No more processing is therefore necessary.

2. Mean of "Total number of Steps Taken Per Day" over all days
--------------------------------------------------------------
...
The creation of an Equivalent ggplot.
```{r ggplot histogram}
library(ggplot2)
ggplot(steps_sum, aes(steps)) + geom_histogram(bins = 10)
```

Plot showing the trend in total number of steps taken per day over the duration of two months.
```{r total steps trend}
library(lubridate)
...
3. Average number of daily activity pattern
-------------------------------------------

Within this section, a time series plot (i.e. type = "1") of the 5-minute interval (x-axis) and the average number of steps
taken averaged across all days of the data series is formally constructed.
...
The time interval or duration during which the maximum number of steps is taken is:
...
4. Inserting the missing values

The main question asked is are there days with all time intervals reporting NA step values?
...
The time interval for each day corresponds to approximately 40 hours, which refutes the intervals being disjoint.
...
Histogram of the total number of steps taken each day with the inserted missing values.
...
5. Differences in activity patterns. Weekends vs Weekdays
---------------------------------------------------------
